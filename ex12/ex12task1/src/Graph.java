...
class Graph {
    ...
    */
    public void breadthFirstSearch (int v) {
		...
		var firstEdges = fromTo[v];
		for (Edge e :firstEdges) {
			if(e != null && e.marker != EDGE_FOUND){
				buffer.enqueue(e);
			}
		}
		vertex[v].marker = VERTEX_FOUND;
       // EXERCISE
		while (!buffer.isEmpty()){
			var edge = (Edge)buffer.dequeue();
			edge.marker = BFS_DISCOVERY_EDGE;
			if(vertex[edge.vertex2].marker == VERTEX_CLEAR){
				var edgeRow = fromTo[edge.vertex2];
				for (Edge e:edgeRow) {
					if(e != null && e.marker == VERTEX_CLEAR){
						buffer.enqueue(e);
					}
				}
				vertex[edge.vertex2].marker = VERTEX_FOUND;
			}
		}
    }
