...
public class PriorityQueue {
	...
	public void insertItem(long key, Object item) {
		_size++;
		var boxed = new PQNode(key,item,null,null);
		var cursor = _first;
		if(cursor == null){
			_first = _last = boxed;
			return;
		...
		while(cursor.getNext() != null){
			if( key <cursor.getKey()){
				boxed.setPrev(cursor.getPrev());
				boxed.setNext(cursor);
				cursor.setPrev(boxed);
				if(boxed.getPrev() == null){
					_first = boxed;
				}else {
					boxed.getPrev().setNext(boxed);
				}
				return;
			}
		...
		if( key>cursor.getKey()) {
			cursor.setNext(boxed);
			boxed.setPrev(cursor);
			_last = boxed;
		}else {
			boxed.setPrev(cursor.getPrev());
			boxed.setNext(cursor);
