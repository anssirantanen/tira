...
class MinHeap {
    ...
    */
    private int percolateDown (int idx) {
        ...
        var cursor = idx;
        while(true){
            var left = leftChild(cursor);
            var right = rightChild(cursor);
            if(left<= last && table[left] <table[cursor]){
                if(right <= last && table[left] > table[right]){
                    swap(cursor,right);
                    cursor= right;
                }else {
                    swap(cursor,left);
            ...
            }else if(right <= last && table[right]< table[cursor]){
                swap(right,cursor);
                cursor=right;
            }else {
                break;
    ...
    */
    private int percolateUp (int idx) {
        ...
        var cursor = idx;
        while (true){
            var parent = parent(cursor);
            if(parent <root){
                break;
            }
            if(table[parent]> table[cursor]){
                swap(parent,cursor);
                cursor = parent;
            }else {
                break;
